...
def evaluate(opt):
    ...
    MODEL_PATHS = ["/root/tmp/20epochs-a/models/weights_19",
                   "/root/tmp/20epochs-b/models/weights_19",
                   "/root/tmp/20epochs-c-retry/models/weights_19"]
    
    MODEL_PATHS = [os.path.expanduser(path) for path in MODEL_PATHS]

    filenames = readlines(os.path.join(splits_dir, opt.eval_split, "test_files.txt"))
    
    models = []

    encoder_path = os.path.join(MODEL_PATHS[0], "encoder.pth")
    ...
    for path in MODEL_PATHS:
        ...
        depth_decoder.cuda()
        depth_decoder.eval()
        models.append((encoder, depth_decoder))
    ...
    plt.title("Mean RMSE vs Mean Per-Pixel Variance with Ensemble Model")
    plt.xlabel("Mean RMSE")
    plt.ylabel("Mean Per-Pixel Variance")
    plt.savefig("assets/per_image_ensemble_plot.png")
    ...
    plt.title("Depth Errors vs Variance of Detph Values with Ensemble Model")
    plt.xlabel("Depth Errors (Absolute Difference)")
    plt.ylabel("Variance of Estimated Detph Values")
    plt.savefig("assets/per_pixel_ensemble_plot.png")
